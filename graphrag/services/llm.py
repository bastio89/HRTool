...
class LLMService:
    ...
    async def _resolve_prompt(self, key: str) -> str:
        if self.prompt_service is None:
            raise RuntimeError(
                f"Prompt-Resolver ist nicht konfiguriert. Benötigter Prompt: {key}"
            )
        try:
            return await self.prompt_service.get_prompt(key)
        except PromptNotFoundError as exc:
            raise RuntimeError(f"Pflicht-Prompt fehlt in der Datenbank: {key}") from exc
        except PromptServiceUnavailableError as exc:
            raise RuntimeError(str(exc)) from exc
        except Exception as exc:
            raise RuntimeError(f"Prompt konnte nicht geladen werden: {key}") from exc

    async def _resolve_json_response_instruction(self) -> str:
        return await self._resolve_prompt("json_only_response_instruction")
        ...
        if provider == "openrouter":
            ...
            request_body = {
                ...
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": f"{await self._resolve_json_response_instruction()}\n\nEingabe:\n{user_content}",
                    },
                ],
                "temperature": 0,
            ...
            request_body = {
                ...
                "messages": [
                    {"role": "system", "content": system_prompt},
                    {
                        "role": "user",
                        "content": (
                            f"{await self._resolve_json_response_instruction()}\n\n"
                            f"Eingabe:\n{user_content}"
                        ),
                    },
                ],
    ...
    async def parse_candidate_cv(self, raw_text: str) -> CandidateProfileExtraction:
        ...
        if not result.work_history:
            recovered_work_history = recover_work_history_from_text(raw_text)
            if recovered_work_history:
                    result.work_history = [WorkHistoryExtraction(**entry) for entry in recovered_work_history]
                    first_entry = result.work_history[0]
                    if not result.current_employer and first_entry.employer:
                        result.current_employer = first_entry.employer
                    if not result.current_position and first_entry.position:
                        result.current_position = first_entry.position
                    if not result.experience:
                        result.experience = "\n".join(
                            filter(
                                None,
                                (
                                    ", ".join(filter(None, (entry.position, entry.employer)))
                                    for entry in result.work_history[:3]
                                ),
                            )
                        )

        if not result.education_history:
            recovered_education_history = recover_education_history_from_text(raw_text)
            if recovered_education_history:
                    result.education_history = [EducationHistoryExtraction(**entry) for entry in recovered_education_history]
