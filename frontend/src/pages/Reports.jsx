...
function OverviewTab({ t }) {
  ...
  if (!data) return <Card className="p-8"><p className="text-gray-500 dark:text-gray-400">{t('reports.error')}</p></Card>
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            <Card key={i} className="p-5">
              ...
              <p className="text-[13px] font-medium text-gray-500 dark:text-gray-400 mt-0.5">{kpi.label}</p>
              <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5">{kpi.sub}</p>
            </Card>
          )
        })}
      </div>
            ...
            return (
              <div key={i} className="text-center">
                ...
                <div className="flex justify-center gap-4 mt-1">
                  <span className="text-[11px] text-gray-500 dark:text-gray-400">{t('reports.conversion')}: {step.conversionRate}%</span>
                  {i > 0 && <span className="text-[11px] text-gray-500 dark:text-gray-400">{t('reports.step_rate')}: {step.stepRate}%</span>}
                </div>
              </div>
            )
          })}
    ...
    <div className="space-y-6 max-w-[1000px]">
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-6 text-center">
          ...
          <p className="text-[14px] text-gray-500 dark:text-gray-400 font-medium">{t('reports.avg_days')}</p>
        </Card>
        <Card className="p-6 text-center">
          <p className="text-[36px] font-bold text-[#5e5ce6]">{data.summary.medianDays ?? '—'}</p>
          <p className="text-[14px] text-gray-500 dark:text-gray-400 font-medium">{t('reports.median_days')}</p>
        </Card>
        <Card className="p-6 text-center">
          <p className="text-[36px] font-bold text-[#34c759]">{data.summary.totalHires}</p>
          <p className="text-[14px] text-gray-500 dark:text-gray-400 font-medium">{t('reports.total_hires')}</p>
        </Card>
      </div>
          ...
          <div className="space-y-3">
            {data.byJob.map((j, i) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-xl bg-[#f5f5f7] dark:bg-[#2c2c2e]">
                ...
                <span className="text-[13px] text-gray-500 dark:text-gray-400">{j.hires} {t('reports.hires')}</span>
              ...
              <tbody>
                {data.recentHires.slice(0, 10).map((h, i) => (
                  <tr key={i} className="border-b border-gray-50 dark:border-gray-700/50">
                    ...
                    <td className="py-2.5 text-gray-500 dark:text-gray-400">{h.job_title}</td>
...
function SourcesTab({ t }) {
  ...
  if (loading) return <LoadingSpinner />
  if (!data || data.length === 0) return <Card className="p-8 text-center"><p className="text-gray-500 dark:text-gray-400">{t('reports.no_data')}</p></Card>
              ...
              {data.map((s, i) => (
                <tr key={i} className="border-b border-gray-50 dark:border-gray-700/50">
                  ...
                  <td className="py-3 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[12px] font-bold ${s.hireRate > 20 ? 'bg-[#34c759]/10 text-[#34c759]' : s.hireRate > 0 ? 'bg-[#ff9f0a]/10 text-[#ff9f0a]' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}>
                      {s.hireRate}%
                    </span>
                  </td>
                </tr>
          ...
          {data.map((d, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3">
                <span className="text-[12px] text-gray-500 dark:text-gray-400 w-20 font-mono">{d.date.slice(5)}</span>
            ...
            <div key={i} className="p-4 rounded-xl bg-[#f5f5f7] dark:bg-[#2c2c2e]">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-gray-500 dark:text-gray-400">@{u.username}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="text-center p-2 bg-white dark:bg-[#1c1c1e] rounded-lg">
                  ...
                  <p className="text-[11px] text-gray-500 dark:text-gray-400">{t('reports.actions')}</p>
                </div>
                <div className="text-center p-2 bg-white dark:bg-[#1c1c1e] rounded-lg">
                  <p className="text-[18px] font-bold text-[#5e5ce6]">{u.comments}</p>
                  <p className="text-[11px] text-gray-500 dark:text-gray-400">{t('reports.comments')}</p>
                </div>
                <div className="text-center p-2 bg-white dark:bg-[#1c1c1e] rounded-lg">
                  <p className="text-[18px] font-bold text-[#ff9f0a]">{u.ai_calls}</p>
                  <p className="text-[11px] text-gray-500 dark:text-gray-400">{t('reports.ai_calls')}</p>
                </div>
              </div>
            </div>
          ))}
